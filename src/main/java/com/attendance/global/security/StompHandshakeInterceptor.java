...
/**
 * STOMP WebSocket 핸드셰이크(최초 연결) 시점에 JWT를 인증하는 인터셉터다.
 * 브라우저 네이티브 WebSocket은 핸드셰이크 요청에 커스텀 헤더를 실을 수 없어(new WebSocket(url)에는
 * 헤더 옵션이 없다) 토큰을 쿼리 파라미터(?token=)로 받아 여기서 직접 검증한다. SecurityConfig가
 * /ws/**를 permitAll로 두고 인증 책임을 이곳으로 옮긴 이유도 그래서다.
 */
@Slf4j
@Component
...
public class StompHandshakeInterceptor implements HandshakeInterceptor {
  ...
  /**
   * 연결 수립 전 호출, false 반환 시 핸드셰이크 자체가 거부된다.
   * 검증에 성공하면 userId/role을 WebSocket 세션 attributes에 저장해, 이후 구독 권한 제한 등에서 재사용한다.
   */
  @Override
