 ...
 *
 * <p>REST API는 매 요청마다 Authorization 헤더로 JWT를 검증하지만(JwtAuthenticationFilter), 브라우저 네이티브 WebSocket은
 * 핸드셰이크 요청에 커스텀 헤더를 실어 보낼 수 없다 (new WebSocket(url)에는 헤더 옵션이 없음). 그래서 토큰을 쿼리 파라미터(?token=)로 받아 여기서
 * 직접 검증한다. SecurityConfig에서 /ws/** 경로를 permitAll로 두고 JwtAuthenticationFilter 대상에서 제외한 것도 인증 책임을 이
 * 인터셉터로 옮겼기 때문이다.
 */
@Slf4j
@Component
...
public class StompHandshakeInterceptor implements HandshakeInterceptor {
   ...
   *
   * <p>검증에 성공하면 userId/role을 WebSocket 세션 attributes에 저장해두고, Phase 2(구독 권한 제한, 이벤트 발행 시 발신자 식별 등)에서
   * 재사용할 수 있게 한다.
   */
  @Override
