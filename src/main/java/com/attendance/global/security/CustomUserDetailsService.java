 ...
 *
 * - Spring Security의 인증 프로제스에서 사용자 정보를 로드하는 서비스
 * - username/userId를 기반으로 데이터베이스에서 사용자 정보를 조회하고 UserDetails 객체로 변환해 반환
 *
 * 동작 흐름:
 * 1. 사용자가 로그인 요청 (username + password)
 * 2. Spring Security가 loadUserByUsername() 호출
 * 3. 데이터베이스에서 사용자 조회
 * 4. CustomUserDetails 객체로 래핑하여 반환
 * 5. Spring Security가 비밀번호 검증 수행
 */
@Service
...
public class CustomUserDetailsService implements UserDetailsService{
     ...
     * - Spring Security의 표준 인증 메커니즘에서 호출되는 메서드
     */
    @Override
    public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
        User user = userRepository.findByUsername(username)
                // @throws UsernameNotFoundException Spring Security 표준 예외 (실제로는 EntityNotFoundException 발생)
     ...
     * - UserDetailSrervice 인터페이스에는 없는 커스텀 메서드
     *
     * JWT 인증 흐름:
     *  1. 클라이언트가 Authorization 헤더에 JWT 토큰 포함하여 요청
     *  2. JwtAuthenticationFilter가 토큰 검증 및 사용자 ID 추출
     *  3. loadUserById(userId) 호출하여 사용자 정보 로드
