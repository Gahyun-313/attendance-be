 ...
 * - 토큰에서 사용자 정보 추출 (userId, username, role)
 * - 토큰 유효성 검증 (서명, 만료시간)
 * - HMAC-SHA256 알고리즘을 사용한 서명 (대칭키, 단일 서버 환경에 적합)
 */
@Slf4j
@Component
public class JwtTokenProvider {
     ...
     * 토큰 유효성 검증
     *
     * parseClaims() 내부에서 아래 항목을 자동으로 검증:
     *  1. 서명 유효성   : secretKey로 HMAC-SHA256 서명 검증
     *  2. 만료 여부    : exp 클레임과 현재 시간 비교
     *  3. 토큰 구조    : header.payload.signature 형식 확인
     *
     * 각 예외별 로그만 남기고 false 반환 (-> 예외를 상위로 전파하지 않음)
     * -> JwtAuthenticationFilter에서 단순 true/false로 분기 처리 가능
     */
    ...
    /**
     * 토큰 파싱 및 Claims 반환 (내부 공통 메서드)
     * - 모든 토큰 정보 추출 메서드와 검증 메서드에서 공통으로 사용
     *
     * [처리 순서]
     * 1. verifyWith(secretKey): HMAC-SHA256으로 서명 검증
     * 2. parseSignedClaims(token): Base64 디코딩 후 파싱
     * 3. getPayload(): 검증된 Claims 객체 반환
     *
     * - private로 선언하여 외부에서 직접 호출 불가
     * - 예외는 호출한 public 메서드에서 처리
     */
