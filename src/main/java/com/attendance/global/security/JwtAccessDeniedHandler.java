...
/**
 * 인가(Authorization) 실패 시 처리하는 핸들러 [403 Forbidden, 인가 실패] - JWT 토큰은 유효하지만 접근 권한이 없음
 *
 * <p>[흐름] 요청 -> JwuAuthenticationFilter (토큰 유효) -> SecurityContext 인증 정보 세팅 완료 -> 권한 체크 실패
 * (@PreAuthorize, SecurityConfig 경로 설정 등) -> Spring Security가 AccessDeniedException 발생 ->
 * JwtAccessDeniedHandler.handle() 호출 -> 403 응답 반환
 */
@Slf4j
@Component
