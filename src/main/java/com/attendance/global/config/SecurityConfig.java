 ...
 *  4. 예외 처리 : 401 -> JwtAuthenticationEntryPoint
 *               403 -> JwtAccessDeniedHandler
...
public class SecurityConfig {
     ...
     * - Spring Security의 핵심 설정으로, 모든 HTTP 요청이 이 체인을 통과
     *
     * [순서]
     *      요청
     *      -> CorsFilter : CORS 검증
     *      -> JwtAuthenticationFilter : JWT 검증 및 SecurityContext 세팅
     *      -> UsernamePasswordAuthenticationFilter : JWT 사용으로 실질적 미사용
     *      -> AuthorizationFilter : 인가 규칙 적용
     *      -> 컨트롤러
     */
    @Bean
     ...
     * 비밀번호 암호화 인코더 빈 등록
     *
     * BCrypt: 단방향 해시 알고리즘, salt 자동 적용
     * - 같은 비밀번호라도 매번 다른 해시값 생성 -> Rainbow Table 공격 방어
     ...
     *
     * 브라우저의 동일 출처 정책(Same-Origin Policy)을 제어, 허용된 Origin에서 API 호출 가능
     */
    @Bean
