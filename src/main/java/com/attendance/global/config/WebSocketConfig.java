 ...
 *
 * <p>[왜 STOMP인가] 순수 WebSocket은 "메시지가 어떤 화면으로 가야 하는지"를 구분하는 개념이 없어 그걸 애플리케이션 코드가 직접 파싱해야 한다. STOMP는
 * 그 위에 pub/sub 프로토콜(destination 경로 기반 SUBSCRIBE/SEND)을 얹어서, 관리자 대시보드가
...
public class WebSocketConfig implements WebSocketMessageBrokerConfigurer {
   ...
   *
   * <p>enableSimpleBroker("/topic") : 서버 -> 클라이언트 방향. "/topic/..."으로 시작하는 목적지를 구독 중인 클라이언트들에게
   * Spring 내장 인메모리 브로커가 그대로 전달한다 (Redis/RabbitMQ 같은 외부 브로커 없이도 단일 서버 환경에서는 충분하다).
   *
   * <p>setApplicationDestinationPrefixes("/app") : 클라이언트 -> 서버 방향. "/app/..."으로 오는 메시지를 컨트롤러의
   * MessageMapping 메서드가 처리한다. 현재는 관리자가 구독만 하고 서버로 보낼 메시지가 없어 당장 쓰이진 않지만, 표준 STOMP 설정 관례상 미리 열어둔다.
   */
  @Override
   ...
   *
   * <p>StompChannelInterceptor가 SUBSCRIBE 프레임의 destination과 role을 확인해, /topic/attendance/** 구독을
   * ADMIN으로 제한한다 (관리자용 실시간 대시보드이므로 STUDENT는 구독할 필요/권한이 없음).
   */
  @Override
