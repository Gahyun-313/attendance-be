...
public interface AttendanceRepository extends JpaRepository<AttendanceRecord, Long> {
  ...
  /**
   * 내 출석 기록 조회 (페이징) - 앱 "내역" 화면용. 정렬은 Controller에서 Pageable(checkInTime DESC 등)로 전달.
   * AttendanceRecord의 (user_id, check_in_time) 복합 인덱스로 필터링+정렬을 함께 커버한다. cursor 기반 페이지네이션은 도입하지 않기로
   * 결정 - 한 사용자의 출석 기록은 학기/그룹 단위로 자연히 개수가 제한적이라(관리자 쪽 무한 스크롤성 대량 데이터와 다름), offset pagination으로도
   * 충분하다고 판단.
   */
   ...
   * AttendanceRecord는 organizationId 컬럼을 직접 갖고 있지 않아(세션을 통해 간접적으로만 소속이 결정됨), sessionId가 해당 단체 소속 세션
   * ID 목록에 포함되는지로 서브쿼리를 걸어 판단한다.
   */
  @Query(
