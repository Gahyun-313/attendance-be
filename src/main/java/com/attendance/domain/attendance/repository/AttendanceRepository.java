...
public interface AttendanceRepository extends JpaRepository<AttendanceRecord, Long> {
  ...
  /** user+session으로 기존 레코드 조회. 체크인 시 WAITING 레코드가 있으면 갱신하고, 없으면 신규 생성할 때 사용한다(AttendanceService.checkIn). */
  Optional<AttendanceRecord> findByUserIdAndSessionId(Long userId, Long sessionId);

  /** 내 출석 기록 페이징 조회. 앱 "내역" 화면에 사용하며, 정렬은 Controller에서 Pageable로 전달받는다. 한 사용자 기록은 학기/그룹 단위라 개수가 제한적이라 offset pagination으로 충분하다. */
  Page<AttendanceRecord> findByUserId(Long userId, Pageable pageable);

  /** 세션별 출석 현황 조회(관리자). 한 세션의 전체 출석 레코드 반환. */
  List<AttendanceRecord> findBySessionId(Long sessionId);

  /** 세션별+상태별 레코드 조회. 세션 종료 시 남은 WAITING을 ABSENT로 일괄 처리할 때 사용(AttendanceService.markAbsentForRemainingWaiting). */
  ...
  /** 단체별+상태별 레코드 수 조회. countByStatus의 단체 격리 버전이며, AttendanceRecord에는 organizationId가 없어 세션을 통해 간접 소속을 서브쿼리로 판단한다. */
  @Query(
