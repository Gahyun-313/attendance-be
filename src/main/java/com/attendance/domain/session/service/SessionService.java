...
public class SessionService {
   ...
   */
  @Transactional
  public SessionResponse createSession(SessionRequest request, Long createdBy, Long organizationId) {
    // NFC 태그 연결 (선택)
   ...
   * - organizationId는 요청한 관리자의 단체로 고정 (다른 단체 세션 노출 방지)
   */
  public Page<SessionResponse> getSessions(
    ...
    if (status != null && keyword != null) {
      return sessionRepository
              .findByOrganizationIdAndStatusAndTitleContaining(organizationId, status, keyword, pageable)
    ...
    return sessionRepository.findByOrganizationId(organizationId, pageable).map(SessionResponse::from);
  }

  /**
   * 세션 상세 조회
   * - 요청한 관리자와 다른 단체 소속 세션이면 존재 자체를 노출하지 않기 위해 조회 실패(404)로 처리
   */
  ...
  /**
   * 세션 수정
   * - 다른 단체 세션이면 존재 자체를 노출하지 않기 위해 404로 처리 (getSession과 동일한 패턴)
   */
  @Transactional
  public SessionResponse updateSession(Long sessionId, SessionRequest request, Long organizationId) {
  ...
  /**
   * 세션 삭제
   * - 다른 단체 세션이면 존재 자체를 노출하지 않기 위해 404로 처리
   */
  @Transactional
  ...
  /**
   * 세션 조회 + 소속 단체 검증 공통 메서드 - 수정/삭제/시작/종료/취소 등 상태를 바꾸는 작업 전에 호출해서,
   * 다른 단체 세션을 조작하려는 시도를 존재 자체가 없는 것처럼(404) 차단한다.
   */
