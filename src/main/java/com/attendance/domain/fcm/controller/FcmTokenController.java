...
/**
 * FCM 토큰 관리 API - 로그인한 사용자라면 STUDENT/ADMIN 구분 없이 누구나 호출 가능. SecurityConfig에 이 경로를 따로 등록하지 않아도
 * anyRequest().authenticated() 규칙에 걸려 "로그인 필요"만 자동으로 적용된다. - userId는 요청 바디로 받지 않는다. 로그인 시 발급한 JWT를
 * 서버가 해석해 만든 인증 정보(@AuthenticationPrincipal)에서 꺼내 쓴다. 요청 바디에 userId를 그대로 받으면 클라이언트가 남의 ID를 넣어 위조할 수
 * 있기 때문.
 */
@RestController
