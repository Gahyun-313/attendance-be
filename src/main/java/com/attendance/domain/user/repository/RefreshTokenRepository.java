...
public interface RefreshTokenRepository extends JpaRepository<RefreshToken, Long> {
   ...
   *
   * <p>{@code @Modifying} 없는 평범한 derived delete 메서드는 "먼저 조회해서 각 엔티티를 remove() 예약"만 해두고 실제 DELETE는
   * 영속성 컨텍스트가 flush될 때(보통 트랜잭션 커밋 시점)에야 나간다. 그런데 AuthService.login()에서 이 메서드 바로 뒤에 호출하는 save()는 새
   * RefreshToken이 IDENTITY 전략이라 save() 즉시 INSERT가 나가버린다 - 즉 "삭제 예약"이 아직 반영 안 된 상태에서 같은 user_id로 새
   * 행을 넣으려다 UNIQUE(user_id) 제약에 걸려 실패하는 버그가 있었다 (재로그인 시 500 에러로 재현됨). {@code @Modifying} + JPQL로 바꿔
   * DELETE를 즉시(동기) 실행되게 해서 해결.
   */
