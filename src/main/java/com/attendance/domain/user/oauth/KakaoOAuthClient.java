 ...
 * 카카오 OAuth 사용자 정보 조회
 *
 * <p>카카오는 구글과 달리 ID Token 방식보다 REST API(카카오 로그인 SDK가 발급한 Access Token으로
 * 사용자 정보 API 호출)가 표준적이라, 프론트에서 받은 Access Token으로 카카오 사용자 정보 API를 직접
 * 호출하는 방식을 쓴다. 이메일은 카카오 앱 설정에서 "이메일" 동의항목을 필수로 설정해둬야 내려온다.
 */
@Slf4j
@Component
