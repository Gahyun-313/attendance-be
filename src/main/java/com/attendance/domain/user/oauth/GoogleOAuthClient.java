 ...
 * 구글 OAuth 사용자 정보 조회
 *
 * <p>프론트가 구글 SDK(Google Identity Services)로 받은 ID Token(JWT)을 그대로 넘겨받아, 구글의 tokeninfo 엔드포인트에 검증을
 * 위임한다. 백엔드가 JWT 서명 검증 라이브러리를 직접 들고 있을 필요가 없어(공개키 캐싱/로테이션 관리 불필요) 구현이 단순해지는 대신, 매 로그인마다 구글에 네트워크
 * 호출이 한 번 발생한다. 로그인 빈도를 생각하면 감수할 만한 트레이드오프로 판단.
 */
@Slf4j
@Component
