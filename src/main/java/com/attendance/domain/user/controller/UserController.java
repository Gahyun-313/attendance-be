...
public class UserController {
  ...
  /**
   * 학생 목록 조회 GET /api/users - ADMIN 전용
   * - groupName: 그룹 필터링 (선택)
   * - keyword: 학번/이름 검색 (선택)
   */
  @PreAuthorize("hasRole('ADMIN')")
  ...
  public ResponseEntity<ApiResponse<UserResponse>> updateUser(
          ...
          @AuthenticationPrincipal CustomUserDetails userDetails) {
    UserResponse response = userService.updateUser(userId, request, userDetails.getOrganizationId());
    return ResponseEntity.ok(ApiResponse.success(response, "사용자 정보가 수정되었습니다"));
  }

  /**
   * 사용자 삭제(비활성화) DELETE /api/users/{userId} - ADMIN 전용
   * - 연관 데이터 보존을 위해 물리 삭제가 아닌 비활성화 처리
   */
  @PreAuthorize("hasRole('ADMIN')")
