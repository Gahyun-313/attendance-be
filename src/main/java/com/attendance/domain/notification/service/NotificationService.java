...
public class NotificationService {
    ...
    /**
     * 알림 생성 (ADMIN)
     * - 저장 직후 발송 대상 시각(scheduledAt)이 지금이거나 없으면 바로 발송 처리(dispatch)까지 수행한다.
     * - 미래 시각으로 예약된 경우 SCHEDULED 상태로만 남고, 실제로 그 시각에 자동 발송해주는 스케줄러는 아직 없다
     *   (추후 Day 6 이후 배치/스케줄러 도입 시점에 "SCHEDULED이면서 scheduledAt이 지난 알림"을 찾아 dispatch하는 방식으로 이어붙일 예정)
     */
    @Transactional
    ...
    /**
     * 알림 목록 조회
     * - ADMIN: 전체 알림을 상태 필터(선택)와 함께 조회 (알림 관리 화면용)
     * - STUDENT: 본인 그룹(또는 전체발송) 대상 + 발송완료(SENT) 알림만 조회 ("내 알림함") - 요청받은 status 파라미터는 무시하고
     *   항상 SENT로 강제한다. 다른 그룹 대상이거나 예약/실패/취소 상태인 알림이 노출되면 안 되기 때문
     */
    ...
    /**
     * 알림 발송 처리
     * - 대상 그룹(또는 전체) 학생들의 FCM 토큰 등록 여부만 확인해서 SENT/FAILED로 전이시킨다.
     * - 실제 FCM 푸시 발송(Firebase Admin SDK의 FirebaseMessaging.send 등 호출)은 build.gradle의 firebase-admin
     *   의존성이 아직 비활성화 상태라 구현하지 않았다 - 의존성 활성화 후 이 메서드 안에 실제 발송 호출을 추가하면 된다.
     */
    private void dispatch(Notification notification) {
        ...
        int tokenCount =
                targets.stream().mapToInt(user -> fcmTokenRepository.findAllByUserId(user.getId()).size()).sum();
