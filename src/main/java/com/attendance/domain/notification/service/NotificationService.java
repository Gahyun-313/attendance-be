...
public class NotificationService {
  ...
  /**
   * 알림 생성(ADMIN)
   * 저장 직후 발송 대상 시각(scheduledAt)이 지금이거나 없으면 바로 dispatch까지 수행한다.
   * 미래 예약이면 SCHEDULED 상태로만 남으며, 자동 발송 스케줄러는 아직 없다(추후 배치 도입 시 이어붙일 예정).
   */
  @Transactional
  ...
  /**
   * 알림 목록 조회
   * ADMIN은 상태 필터(선택)를 포함해 전체를 조회한다. STUDENT는 본인 그룹(또는 전체발송)이면서 SENT인
   * 알림만 "내 알림함"으로 조회하며, status 파라미터는 무시하고 예약/실패/취소 상태 노출을 막기 위해 항상 SENT로 고정한다.
   */
  ...
  /**
   * 알림 발송 처리
   * 실제 FCM 푸시 발송은 firebase-admin 의존성이 비활성화 상태라 아직 구현하지 않았고,
   * 대상 학생들의 FCM 토큰 등록 여부만 확인해 SENT/FAILED로 전이시킨다.
   */
