...
/**
 * 알림 엔티티
 * - 관리자가 만든 "발송 단위" 하나를 표현한다 (수신자별 읽음 여부 등 개인화된 내역은 별도 테이블 없이,
 *   targetGroup 조건으로 학생이 자신에게 해당하는 알림만 걸러보는 방식으로 근사한다 - NotificationRepository.findVisibleToGroup 참고)
 * - 실제 FCM 푸시 발송은 firebase-admin 의존성이 아직 비활성화 상태라 이번 Phase에서는 대상자의 토큰 존재 여부만 확인하고
 *   SENT/FAILED로 전이시키는 것까지만 구현한다 (NotificationService.dispatch 참고)
 */
@Entity
@Table(
        name = "notifications",
