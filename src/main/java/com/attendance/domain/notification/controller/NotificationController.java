...
public class NotificationController {
    ...
    /**
     * 알림 생성 POST /api/notifications - ADMIN 전용
     * - scheduledAt이 없거나 지난 시각이면 생성 즉시 발송 시도, 미래 시각이면 SCHEDULED로 남음
     */
     ...
     * - ADMIN: 전체 알림 (status 필터 선택)
     * - STUDENT: 본인 그룹(또는 전체발송) 대상의 발송완료 알림만 - status 파라미터는 무시됨
     */
    @GetMapping
    ...
    /**
     * 알림 취소 DELETE /api/notifications/{notificationId} - ADMIN 전용
     * - 아직 발송 전(SCHEDULED)인 알림만 취소 가능
     */
    @PreAuthorize("hasRole('ADMIN')")
