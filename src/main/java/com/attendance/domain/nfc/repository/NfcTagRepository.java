...
public interface NfcTagRepository extends JpaRepository<NfcTag, Long> {
    ...
    /**
     * UID로 NFC 태그 조회
     * - AOS 앱에서 NFC 스캔 시 UID로 태그 검증에 사용
     */
    ...
    /**
     * 상태별 NFC 태그 조회 (페이징)
     * - 관리자 화면에서 활성/비활성 태그 조회에 사용
     */
    Page<NfcTag> findByStatus(NfcTagStatus status, Pageable pageable);

    /**
     * 이름/위치로 NFC 태그 검색 (페이징)
     * - 관리자 화면에서 태그 검색에 사용
     */
    Page<NfcTag> findByNameContainingOrLocationContaining(String name, String location, Pageable pageable);
}
