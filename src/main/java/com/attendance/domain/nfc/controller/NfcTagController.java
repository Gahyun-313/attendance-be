...
public class NfcTagController {
    ...
    public ResponseEntity<ApiResponse<NfcTagResponse>> createNfcTag(
        ...
        NfcTagResponse response = nfcTagService.createNfcTag(request);
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(ApiResponse.success(response, "NFC 태그가 등록되었습니다"));
    }

    /**
     * NFC 태그 목록 조회 (페이징) GET /api/nfc-tags
     * - 전체: status 파라미터 없음
     * - 상태별: ?status=ACTIVE
     */
    @GetMapping
    public ResponseEntity<ApiResponse<Page<NfcTagResponse>>> getNfcTags(
        ...
        Page<NfcTagResponse> response =
                status != null
                        ? nfcTagService.getNfcTagsByStatus(status, pageable)
                        : nfcTagService.getAllNfcTags(pageable);
     ...
     */
    @GetMapping("/{tagId}")
    public ResponseEntity<ApiResponse<NfcTagResponse>> getNfcTag(@PathVariable Long tagId) {
        NfcTagResponse response = nfcTagService.getNfcTagById(tagId);
        return ResponseEntity.ok(ApiResponse.success(response, "NFC 태그 조회 성공"));
    }

    /**
     * UID로 NFC 태그 조회 GET /api/nfc-tags/uid/{uid}
     * - AOS 앱에서 NFC 스캔 후 태그 검증에 사용
     */
    @GetMapping("/uid/{uid}")
    ...
    /**
     * NFC 태그 수정 PUT /api/nfc-tags/{tagId}
     * - ADMIN 전용
     */
    @PreAuthorize("hasRole('ADMIN')")
    @PutMapping("/{tagId}")
    public ResponseEntity<ApiResponse<NfcTagResponse>> updateNfcTag(
            @PathVariable Long tagId, @Valid @RequestBody NfcTagUpdateRequest request) {
        NfcTagResponse response = nfcTagService.updateNfcTag(tagId, request);
        return ResponseEntity.ok(ApiResponse.success(response, "NFC 태그가 수정되었습니다"));
    }

    /**
     * NFC 태그 활성화 POST /api/nfc-tags/{tagId}/activate
     * - ADMIN 전용
     */
    @PreAuthorize("hasRole('ADMIN')")
    @PostMapping("/{tagId}/activate")
    public ResponseEntity<ApiResponse<NfcTagResponse>> activateNfcTag(@PathVariable Long tagId) {
        NfcTagResponse response = nfcTagService.activateNfcTag(tagId);
        return ResponseEntity.ok(ApiResponse.success(response, "NFC 태그가 활성화되었습니다"));
    }

    /**
     * NFC 태그 비활성화 POST /api/nfc-tags/{tagId}/deactivate
     * - ADMIN 전용
     */
    @PreAuthorize("hasRole('ADMIN')")
    @PostMapping("/{tagId}/deactivate")
    public ResponseEntity<ApiResponse<NfcTagResponse>> deactivateNfcTag(@PathVariable Long tagId) {
        NfcTagResponse response = nfcTagService.deactivateNfcTag(tagId);
        return ResponseEntity.ok(ApiResponse.success(response, "NFC 태그가 비활성화되었습니다"));
    }

    /**
     * NFC 태그 삭제 DELETE /api/nfc-tags/{tagId}
     * - ADMIN 전용
     */
    @PreAuthorize("hasRole('ADMIN')")
    @DeleteMapping("/{tagId}")
    public ResponseEntity<ApiResponse<Void>> deleteNfcTag(@PathVariable Long tagId) {
        nfcTagService.deleteNfcTag(tagId);
        return ResponseEntity.ok(ApiResponse.success("NFC 태그가 삭제되었습니다"));
    }

    /**
     * NFC 태그 검증 POST /api/nfc-tags/validate
     * - AOS 앱에서 출석 체크 전 태그 유효성 검증에 사용
     */
