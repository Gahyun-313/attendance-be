...
/**
 * AttendanceRepository 쿼리 메서드 테스트
 *
 * @DataJpaTest로 인메모리 H2 DB에 실제 AttendanceRecord를 저장하고,
 * checkIn()/markAbsentForRemainingWaiting()이 의존하는 조회 메서드들이 정확한 결과를 주는지 검증한다.
 *
 * 검증하는 주요 정책:
 * - user+session 조합으로 기존 레코드를 정확히 찾는다 (findByUserIdAndSessionId)
 * - 세션+상태 조합으로 필터링한다 (findBySessionIdAndStatus)
 * - user+session 레코드 존재 여부를 정확히 판별한다 (existsByUserIdAndSessionId)
...
class AttendanceRepositoryTest {
    ...
    /**
     * @Autowired 실제 DB 대신 인메모리 H2로 자동 구성된 AttendanceRepository 빈을 주입받는다.
