...
/**
 * SessionRepository 커스텀 쿼리 테스트
 *
 * @DataJpaTest로 인메모리 H2 DB에 실제 AttendanceSession을 저장하고,
 ...
 *
 * 검증하는 주요 정책:
 * - 현재 시각이 세션 시간 범위 안일 때만 활성 세션으로 조회된다 (findActiveSessionsByNfcTagId)
 * - 시작 전/종료 후 세션은 ACTIVE 상태여도 조회되지 않는다
 * - 동일 태그에 여러 활성 세션이 걸리면 startTime 내림차순으로 정렬된다 (방어 로직 검증)
 * - 태그+상태로 세션을 필터링한다 (findByNfcTagIdAndStatus, 중복 ACTIVE 세션 방지에 사용)
 * - 날짜/상태별 세션 수를 정확히 카운트한다 (countBySessionDate, countByStatus)
 * - 그룹+상태로 세션을 필터링한다 (findByGroupNameAndStatus)
 * - 완료된 세션을 최신순으로, 지정한 개수만큼 조회한다 (findByStatusOrderBySessionDateDesc)
 */
@DataJpaTest
class SessionRepositoryTest {

    /**
     * @Autowired 실제 DB 대신 인메모리 H2로 자동 구성된 SessionRepository 빈을 주입받는다.
    ...
    private AttendanceSession saveSession(
            NfcTag tag, String groupName, SessionStatus status, LocalDateTime startTime, LocalDateTime endTime) {
    ...
    class FindActiveSessionsByNfcTagId {
        ...
        void withinTimeRange_returnsSession() {
            ...
            saveSession(
                    tag, "A반", SessionStatus.ACTIVE, LocalDateTime.now().minusMinutes(10), LocalDateTime.now().plusMinutes(50));
        ...
        void startTimeInFuture_returnsEmpty() {
            ...
            NfcTag tag = saveTag("TAG-001");
            saveSession(
                    tag, "A반", SessionStatus.ACTIVE, LocalDateTime.now().plusMinutes(10), LocalDateTime.now().plusMinutes(60));
    ...
    class FindByNfcTagIdAndStatus {
        ...
        void filtersOnlyMatchingStatus() {
            ...
            AttendanceSession activeSession =
                    saveSession(
                            tag, "A반", SessionStatus.ACTIVE, LocalDateTime.now().minusMinutes(5), LocalDateTime.now().plusMinutes(30));
            saveSession(
                    tag,
                    "B반",
    ...
    class CountQueries {
        ...
        void countBySessionDate_countsOnlyMatchingDate() {
            ...
            saveSession(
                    tag, "A반", SessionStatus.SCHEDULED, today.atTime(9, 0), today.atTime(10, 0));
            saveSession(
                    tag, "B반", SessionStatus.SCHEDULED, today.minusDays(1).atTime(9, 0), today.minusDays(1).atTime(10, 0));
        ...
        void countByStatus_countsOnlyMatchingStatus() {
            ...
            NfcTag tag = saveTag("TAG-001");
            saveSession(
                    tag, "A반", SessionStatus.COMPLETED, LocalDateTime.now().minusDays(1), LocalDateTime.now().minusDays(1).plusHours(1));
            saveSession(
                    tag, "B반", SessionStatus.COMPLETED, LocalDateTime.now().minusDays(2), LocalDateTime.now().minusDays(2).plusHours(1));
            saveSession(
                    tag, "C반", SessionStatus.SCHEDULED, LocalDateTime.now().plusDays(1), LocalDateTime.now().plusDays(1).plusHours(1));
    ...
    class FindByGroupNameAndStatus {
        ...
        void filtersOnlyMatchingGroupAndStatus() {
            ...
            saveSession(
                    tag, "A반", SessionStatus.COMPLETED, LocalDateTime.now().minusDays(1), LocalDateTime.now().minusDays(1).plusHours(1));
            saveSession(
                    tag, "A반", SessionStatus.SCHEDULED, LocalDateTime.now().plusDays(1), LocalDateTime.now().plusDays(1).plusHours(1));
            saveSession(
                    tag, "B반", SessionStatus.COMPLETED, LocalDateTime.now().minusDays(1), LocalDateTime.now().minusDays(1).plusHours(1));
