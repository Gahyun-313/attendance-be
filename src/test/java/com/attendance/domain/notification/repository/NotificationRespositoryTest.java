...
/**
 * NotificationRepository 커스텀 쿼리 테스트
 *
 * @DataJpaTest로 인메모리 H2 DB에 실제 Notification을 저장하고,
 * 상태 필터(findByStatus)와 학생 노출 범위 쿼리(findVisibleToGroup)가 정확히 동작하는지 검증한다.
 *
 * 검증하는 주요 정책:
 * - 상태별로 정확히 필터링된다 (findByStatus)
 * - 전체발송(targetGroup null) 알림은 어떤 그룹명으로 조회해도 노출 대상에 포함된다 (findVisibleToGroup)
 * - 특정 그룹 대상 알림은 해당 그룹명일 때만 노출되고, 다른 그룹명이면 제외된다
 * - SENT가 아닌 알림은 대상 그룹이 맞아도 학생에게 노출되지 않는다
 */
@DataJpaTest
class NotificationRepositoryTest {
    ...
    private Notification save(String title, String targetGroup, NotificationStatus status) {
        ...
        Notification notification =
                Notification.builder().title(title).content("내용").targetGroup(targetGroup).createdBy(1L).build();
    ...
    class FindVisibleToGroup {
        ...
        void broadcastNotification_visibleToAnyGroup() {
            ...
            // when
            Page<Notification> result =
                    notificationRepository.findVisibleToGroup(NotificationStatus.SENT, "A반", PageRequest.of(0, 10));
        ...
        void groupSpecificNotification_hiddenFromOtherGroup() {
            ...
            // when
            Page<Notification> result =
                    notificationRepository.findVisibleToGroup(NotificationStatus.SENT, "B반", PageRequest.of(0, 10));
